var ES6_KEYWORDS, KEYWORDS, KEYWORDS_ATOM, RESERVED_WORDS, KEYWORDS_BEFORE_EXPRESSION, ALL_KEYWORDS, OPERATOR_CHARS, RE_HEX_NUMBER, RE_OCT_NUMBER, RE_DEC_NUMBER, OPERATORS, OP_MAP, WHITESPACE_CHARS, PUNC_BEFORE_EXPRESSION, PUNC_CHARS, REGEXP_MODIFIERS, UNICODE, IDENTIFIER_PAT, STRING_MODIFIERS, UNARY_POSTFIX, EX_EOF;
...
function tokenizer($TEXT, filename) {
    var ՐՏ_Temp, S, read_string, read_multiline_comment, read_regexp;
    S = {
        text: $TEXT.replace(/\r\n?|[\n\u2028\u2029]/g, "\n").replace(/\uFEFF/g, ""),
        filename: filename,
        pos: 0,
    ...
    function token(full_type, value, is_comment, keep_newline) {
        var ՐՏ_Temp, full_type, type, subtype, value, ret, i;
    ...
    function parse_whitespace() {
        var ՐՏ_Temp, leading_whitespace, whitespace_exists, ch;
        leading_whitespace = "";
    ...
    function read_num(prefix) {
        var ՐՏ_Temp, has_e, after_e, has_x, has_dot, num, valid;
        has_e = false;
        after_e = false;
        has_x = false;
        has_dot = prefix === ".";
        num = read_while(function(ch, i) {
            var ՐՏ_Temp, code, tmp_, has_x, has_e, has_dot;
            code = ch.charCodeAt(0);
            tmp_ = code;
            if (tmp_ === 120 || tmp_ === 88) {
    ...
    read_string = with_eof_error("Unterminated string constant", function(modifier) {
        var ՐՏ_Temp, token_type, quote, ret, i, tmp, find_newlines, ch;
        token_type = "string";
        if (modifier) {
            token_type += ":" + modifier;
        }
    ...
    function read_line_comment(shebang) {
        shebang = shebang === undefined ? false : shebang;
        var ՐՏ_Temp, i, ret;
        if (!shebang) {
            next();
        }
    ...
    function read_name() {
        var ՐՏ_Temp, backslash, name, escaped, ch, hex;
        backslash = false;
        name = "";
        escaped = false;
        while ((ch = peek()) !== null) {
    ...
    read_regexp = with_eof_error("Unterminated regular expression", function(regexp) {
        var ՐՏ_Temp, prev_backslash, in_class, verbose_regexp, in_comment, mods, ch, regexp;
    ...
    function next_token(force_regexp) {
        var ՐՏ_Temp, indent, ch, code, tmp_, regex_allowed, tok, mods, string_tok;
