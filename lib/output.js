...
(function() {
    ...
    DEFPRINT(AST_ListComprehension, function(self, output) {
        ...
        output.with_parens(function() {
            ...
            output.with_block(function() {
                ...
                output.space();
                output.with_parens(function() {
                    output.spaced("var", "ՐՏ_Index", "=", "0");
                    output.semicolon();
                    output.space();
                    output.spaced("ՐՏ_Index", "<", "ՐՏ_Iter.length");
                    output.semicolon();
                    output.space();
    ...
    AST_Class.DEFMETHOD("_do_print", function(output) {
        var ՐՏupk3, ՐՏ_2;
        var self, define_method, methods, staticmethods, name;
        self = this;
        if (self.external) {
            return;
        }
        ...
        }, ՐՏ_2 = unify(output, name, addInheritance(), addDecorators(), methods, staticmethods)(ՐՏ_2), ՐՏ_2);
        generateClass();
    });
    ...
    DEFPRINT(AST_BaseCall, function(self, output) {
        var object, has_kwarg_items, has_kwarg_formals, has_kwargs, obj, output_kwargs;
        function call_format() {
            ...
            if (self instanceof AST_ClassCall) {
                ...
                rename = ՐՏ_in(self.expression.name, SPECIAL_METHODS) ? SPECIAL_METHODS[self.expression.name] : undefined;
                if (rename) {
                    output.print(rename);
                } else {
                    self.expression.print(output);
    ...
    DEFPRINT(AST_DeepEquality, function(self, output) {
        ...
        if (ՐՏ_in(self.left.computedType, primitives) || ՐՏ_in(self.right.computedType, primitives)) {
            ...
            output.with_parens(function() {
                if (self.operator === "==") {
                    output.spaced(self.left, "===", self.right);
                    output.space();
                    output.spaced("||", "typeof", self.left, "===", '"object"');
                    output.space();
                    output.print("&&");
                    output.space();
                    output.print("ՐՏ_eq");
    ...
    DEFPRINT(AST_Range, function(self, output) {
        ...
        if (self.left instanceof AST_Number && self.right instanceof AST_Number && Math.abs(self.right.value - self.left.value) < 50) {
            ...
            output.with_square(function() {
                var ՐՏitr14, ՐՏidx14;
                var i;
                ՐՏitr14 = ՐՏ_Iterable(range(start, end, step));
