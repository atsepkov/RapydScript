...
(function() {
    ...
    function print_return_map_var(output, name, j) {
        output.newline();
        if (j > 0) {
            output.comma();
        }
    ...
    function print_amd(self, output) {
        var imports, module_name, i, package_name, j;
        imports = [];
        output.print("define('");
        ...
        output.print("function(");
        j = 0;
        imports.forEach(function(node, i) {
            var j;
            output.print(node.path.replace("/", "_"));
            if (j > 0) {
                output.comma();
            }
            j += 1;
            ...
            self.body.forEach(function(node) {
                ...
                if (node.TYPE === "Import") {
                    ...
                    for (var _$rapyd$_Index0 = 0; _$rapyd$_Index0 < _$rapyd$_Iter0.length; _$rapyd$_Index0++) {
                        ...
                        j = print_return_map_var(output, name.name, j);
                    }
                    return;
                }
                name = node.name.name;
                j = print_return_map_var(output, name, j);
            });
            output.newline();
            output.indent();
            output.print("};");
    ...
    DEFPRINT(AST_Import, function(self, output) {
        ...
        if (output.option("amd") && self.argnames) {
            self.argnames.forEach(function(arg) {
                ...
                output.space();
                output.assign(arg);
                output.print(self.path.replace("/", "_"));
                output.print(".");
                arg.print(output);
                output.semicolon();
                output.newline();
