...
function OutputStream(options) {
    ...
    function prologue(module_) {
        ...
        if (!options.omit_baselib) {
            var _$rapyd$_Iter0 = dir(module_.baselib);
...
(function() {
    ...
    function bind_methods(methods, output) {
        var arg;
        var _$rapyd$_Iter1 = dir(methods);
    ...
    function display_complex_body(node, is_toplevel, output) {
        ...
        if (node instanceof AST_Scope) {
            if (node.argnames) {
                ...
                var _$rapyd$_Iter2 = dir(node.argnames.defaults);
