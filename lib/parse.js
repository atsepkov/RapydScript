...
function parse($TEXT, options) {
    ...
    function do_import(key) {
        ...
        modpath = key.replace(".", "/");
        var ՐՏ_Iter11 = ՐՏ_Iterable([ options.basedir, options.libdir ]);
    ...
    function_ = function(in_class, ctor) {
        ...
        nonlocals.forEach(function(variable) {
            var i;
            var ՐՏ_Iter14 = ՐՏ_Iterable(dir(definition.localvars).reverse());
    ...
    function expr_list(closing, allow_trailing_comma, allow_empty, func_call) {
        ...
        if (func_call) {
            tmp = [];
            tmp.kwargs = [];
            var ՐՏ_Iter15 = ՐՏ_Iterable(enumerate(a));
