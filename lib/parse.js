...
var RESERVED_WORDS = "abstract boolean byte char double enum export extends final float goto implements int interface long native package private protected public short static super synchronized this throws transient volatile" + " " + KEYWORDS_ATOM + " " + KEYWORDS;
var KEYWORDS_BEFORE_EXPRESSION = "return new del raise elif else if";
...
var OPERATORS = makePredicate([ "in", "instanceof", "typeof", "new", "void", "del", "++", "--", "+", "-", "not", "~", "&", "|", "^", "*", "/", "%", ">>", "<<", ">>>", "<", ">", "<=", ">=", "==", "is", "!=", "!==", "?", "=", "+=", "-=", "/=", "*=", "%=", ">>=", "<<=", ">>>=", "|=", "^=", "&=", "and", "or", "til", "to", "@" ]);
var OP_MAP = {
    "or": "||",
    "and": "&&",
    "not": "!",
...
function tokenizer($TEXT, filename) {
    ...
    function token(type, value, is_comment, keep_newline) {
        ...
        if (!is_comment) {
            ...
            S.comments_before = [];
            var len_ = ret.comments_before.length;
            for (i = 0; i < len_; i++) {
    ...
    read_regexp = with_eof_error("Unterminated regular expression", function(regexp) {
        var in_class, ch, prev_backslash, mods;
...
function parse($TEXT, options) {
        ...
        if (tmp_ == "string") {
            ...
            if (tmp_ == "break") {
                ...
                return new AST_Return({
                    value: is_("punc", ";") ? (next(), null) : can_insert_semicolon() ? null : function() {
                        var tmp;
                        tmp = expression(true);
                        semicolon();
                        return tmp;
    ...
    function_ = function(in_class, ctor) {
        ...
        expect("(");
        if (!ctor) {
            ctor = in_class ? AST_Defun : AST_Function;
        }
        definition = new ctor({
            name: name,
        ...
        if (definition instanceof AST_Defun) {
    ...
    function expr_list(closing, allow_trailing_comma, allow_empty, allow_starargs) {
        ...
        while (!is_("punc", closing)) {
            ...
            if (is_("operator", "*") && allow_starargs) {
                saw_starargs = true;
                next();
            }
