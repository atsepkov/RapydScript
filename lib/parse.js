...
function parse($TEXT, options) {
    ...
    function get_class_in_scope(expr) {
        var s, referenced_path, module_id, class_name, mod;
        if (expr instanceof AST_SymbolRef) {
            ...
            if (expr instanceof AST_SymbolRef) {
                ...
                if (len(referenced_path) > 1) {
                    module_id = referenced_path.slice(0, -1).join(".");
                    if (S.modules.hasOwnProperty(module_id)) {
                        class_name = referenced_path[referenced_path.length-1];
                        mod = IMPORTED[module_id];
                        if (mod.classes.hasOwnProperty(class_name)) {
                            return mod.classes[class_name];
                        }
                    }
                }
            }
    ...
    import_ = function(from_import) {
        var _$rapyd$_Unpack, name, key, argnames, classes, obj, argvar;
