...
function parse($TEXT, options) {
    var S, statement, find_module_realpath, read_module_contents, import_, from_import_, ModuleTreeNode, module_, class_, function_, nonlocal_, const_, new_, expr_atom, array_, object_, subscripts, maybe_unary, expr_op, maybe_conditional, maybe_assign, expression;
    ...
    find_module_realpath = function(name) {
        var file, search_paths, module_name, fullpath, path;
        file = name;
        ...
        for (var _$rapyd$_Index0 = 0; _$rapyd$_Index0 < _$rapyd$_Iter0.length; _$rapyd$_Index0++) {
            ...
            if (path && path != "") {
                module_name = fullpath = path + "/" + file;
            } else {
                fullpath = options.basedir + "/" + file;
            }
            if (options.fs.existsSync(fullpath + options.py_ext)) {
                sys.log(options.basedir + "] Exists: " + fullpath);
                return fullpath;
            }
        }
        throw new Error("Cannot import: " + name);
    };
    read_module_contents = function(name) {
        var file, search_paths, options_, last_e, fullpath, contents, path;
        file = name + options.py_ext;
        sys.log("amd: " + options.amd + " import: " + file);
        contents = null;
        search_paths = options.search_paths;
        options_ = {
            filename: file,
            ...
            py_ext: options.py_ext,
            amd: options.amd,
            search_paths: search_paths
        };
        last_e = null;
        var _$rapyd$_Iter1 = search_paths;
        for (var _$rapyd$_Index1 = 0; _$rapyd$_Index1 < _$rapyd$_Iter1.length; _$rapyd$_Index1++) {
            path = _$rapyd$_Iter1[_$rapyd$_Index1];
            try {
                if (path && path != "") {
                    fullpath = path + "/" + file;
                } else {
                ...
                contents = parse(options.fs.readfile(fullpath, "utf-8"), options_);
                sys.log("Read: " + fullpath);
            } catch (_$rapyd$_Exception) {
                var e = _$rapyd$_Exception;
                sys.error("Not found: " + fullpath);
                last_e = e;
                continue;
            }
        }
    ...
    function_ = function(in_class, ctor) {
        ...
        nonlocals.forEach(function(variable) {
            var i;
            var _$rapyd$_Iter2 = dir(definition.localvars).reverse();
    ...
    function expr_list(closing, allow_trailing_comma, allow_empty, func_call) {
        ...
        if (func_call) {
            tmp = [];
            tmp.kwargs = [];
            var _$rapyd$_Iter3 = enumerate(a);
